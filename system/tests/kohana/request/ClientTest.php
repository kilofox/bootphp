 ...
 */
class Bootphp_Request_ClientTest extends Unittest_TestCase
{
    ...
    {
        return $this->_dummy_uri($status, array('Location' => $this->_dummy_uri(200, null, 'followed')), 'not-followed');
    }
    ...
    {
        $response = Request::factory($request_url, array('follow' => $follow))
            ->execute();
    ...
    public function test_follows_with_headers()
    {
        $response = Request::factory(
                $this->_dummy_redirect_uri(301), array(
                'follow' => true,
    ...
    {
        $response = Request::factory($this->_dummy_redirect_uri($status_code), array(
                'follow' => true,
    ...
    {
        $response = Request::factory($this->_dummy_redirect_uri($status), array('follow' => true))
    ...
    public function provider_triggers_header_callbacks()
    {
        return array(
            ...
            array(
                ...
                $this->_dummy_uri(200, array('X-test-2' => $this->_dummy_uri(200, null, 'test2-subsequent-body')), 'test2-orig-body'),
            ...
            array(
                ...
                $this->_dummy_uri(200, array(
                    'X-test-1' => $this->_dummy_uri(
                        200, array('X-test-2' => $this->_dummy_uri(200, null, 'test2-subsequent-body')), 'test1-subsequent-body'),
                    ), 'test-orig-body'),
                'test2-subsequent-body'
            ),
        );
    ...
    {
        $response = Request::factory($uri, array('header_callbacks' => $callbacks))
            ->execute();
    ...
    public function test_client_can_hold_params_for_callbacks()
    {
        ...
        $request = Request::factory(
                $this->_dummy_uri(
                    302, array('Location' => $this->_dummy_uri('200', array('X-cb' => '1'), 'followed')), 'not-followed'), array(
                'follow' => true,
                'header_callbacks' => array(
