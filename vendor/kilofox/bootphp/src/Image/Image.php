...
abstract class Bootphp_Image
{
    ...
    public static function factory($file, $driver = null)
    {
        if ($driver === null) {
            ...
            $configured_driver = Core::$config->load('image.default_driver');
    ...
    public function __construct($file)
    {
        ...
        if (empty($file) or empty($info)) {
            throw new BootphpException('Not an image or invalid image: :file', array(':file' => Debug::path($file)));
        }
    ...
    public function save($file = null, $quality = 100)
    {
        ...
        if (is_file($file)) {
            if (!is_writable($file)) {
                throw new BootphpException('File must be writable: :file', array(':file' => Debug::path($file)));
            }
        } else {
            ...
            if (!is_dir($directory) or ! is_writable($directory)) {
                throw new BootphpException('Directory must be writable: :directory', array(':directory' => Debug::path($directory)));
            }
        }
