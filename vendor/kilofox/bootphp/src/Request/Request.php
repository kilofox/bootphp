...
class Request implements \Bootphp\Http\Request
{
    ...
    public static function detect_uri()
    {
        if (!empty($_SERVER['PATH_INFO'])) {
            ...
            if (isset($_SERVER['REQUEST_URI'])) {
                ...
                // If you ever see this error, please report an issue at http://dev.bootphpphp.com/projects/bootphp3/issues
                // along with any relevant information about your web server setup. Thanks!
                throw new BootphpException('Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL');
            }
    ...
    /**
     * Provides access to the [\Bootphp\Request\Client].
     *
    ...
    public function execute()
    {
        ...
        if (!$this->_route instanceof Route) {
            return HttpException::factory(404, 'Unable to find a route to match the URI: :uri', array(':uri' => $this->_uri))
                    ->request($this)
                    ->get_response();
        }

        if (!$this->_client instanceof Client) {
            throw new BootphpException('Unable to execute :uri without a \Bootphp\Request\Client', array(
            ':uri' => $this->_uri,
            ));
        }
