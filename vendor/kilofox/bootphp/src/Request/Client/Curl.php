...
class Bootphp_Request_Client_Curl extends Request_Client_External
{
    ...
    public function _send_message(Request $request, Response $response)
    {
        ...
        if (!curl_setoptArray($curl, $options)) {
            throw new BootphpException('Failed to set CURL options, check CURL documentation: :url', array(':url' => 'http://php.net/curl_setopt_array'));
        }
        ...
        if (isset($error)) {
            throw new BootphpException('Error fetching remote :url [ status :code ] :error', array(':url' => $request->url(), ':code' => $code, ':error' => $error));
        }
