...
/**
 * The Bootphp_HTTP_Header class provides an Object-Orientated interface
...
class Header extends \ArrayObject
{
    ...
    /**
     * Constructor method for [Bootphp_HTTP_Header]. Uses the standard constructor
     * of the parent `ArrayObject` class.
     *
     *     $header_object = new HTTP_Header(array('x-powered-by' => 'Bootphp 3.1.x', 'expires' => '...'));
     *
     ...
     *
     * @param   \Bootphp\Http\Response      $response   Header to send
     * @param   boolean                     $replace    Replace existing value
     * @param   callback                    $callback   Optional callback to replace PHP header function
     * @return  mixed
     */
    public function send_headers(\Bootphp\Http\Response $response = null, $replace = false, $callback = null)
    {
        $protocol = $response->protocol();
        $status = $response->status();

        // Create the response header
        $processed_headers = array($protocol . ' ' . $status . ' ' . \Bootphp\Response::$messages[$status]);
