 ...
 */
class Bootphp_Unittest_Helpers
{
    /**
    ...
    /**
     * Removes all cache files from the bootphp cache dir
     *
     * @return void
     */
    static public function clean_cache_dir()
    {
        $cache_dir = opendir(Core::$cache_dir);
        ...
        while ($dir = readdir($cache_dir)) {
            ...
            if ($dir[0] !== '.' and strlen($dir) === 2) {
                $dir = self::dir_separator(Core::$cache_dir . '/' . $dir . '/');
    ...
    public function set_environment(array $environment)
    {
        ...
        foreach ($environment as $option => $value) {
            ...
            // Else we assume this is a config option
                ...
                if ($backup_needed) {
                    $this->_environment_backup[$option] = Core::$config->load($option);
                }

                list($group, $var) = explode('.', $option, 2);

                Core::$config->load($group)->set($var, $value);
            }
        }
    }
     ...
     *
     * @chainable
     * @return Bootphp_Unittest_Helpers $this
     */
