 ...
 */
class Bootphp_RequestTest extends Unittest_TestCase
{
    ...
    public function test_param()
    {
        ...
        $route->defaults(array('controller' => 'bootphp_requesttest_dummy', 'action' => 'foobar'));
        $request = Request::factory('bootphp_requesttest_dummy', null, true, array($route));
        ...
        $this->assertSame(200, $response->status());
        $this->assertSame($controller->get_expected_response(), $response->body());
        $this->assertSame('bootphp_requesttest_dummy', $request->param('uri'));
    }
    ...
    public function provider_url()
    {
        return array(
            array(
                'foo/bar',
                'http',
                'http://localhost/bootphp/foo/bar'
            ),
            array(
                'foo',
                'http',
                'http://localhost/bootphp/foo'
            ),
            array(
                'http://www.google.com',
                'http',
    ...
    public function test_url($uri, $protocol, $expected)
    {
        ...
        $this->assertEquals(Request::factory($uri, [], true, array($route))->url($protocol), $expected);
    }
    ...
    public function provider_uri_only_trimed_on_internal()
    {
        ...
        Request::$initial = new Request(true, [], true, array($route));
    ...
    {
        $x_powered_by = 'Bootphp Unit Test';
    ...
    public function test_content_length_after_render()
    {
        $request = Request::factory('https://example.org/post')
            ->client(new Bootphp_RequestTest_Header_Spying_Request_Client_External)
            ->method(Request::POST)
            ->post(array('aaa' => 'bbb'));
 ...
 * of the request
 */
class Bootphp_RequestTest_Header_Spying_Request_Client_External extends Request_Client_External
{
    private $headers;
...
class Controller_Bootphp_RequestTest_Dummy extends Controller
{
