 ...
 */
class Bootphp_ConfigTest extends Unittest_TestCase
{
    /**
    ...
    /**
     * Test that calling attach() on a bootphp config object
    ...
    {
        $config = new Config;
        $reader = $this->getMock('Bootphp_Config_Reader');
    ...
    public function test_attach_adds_reader_to_front_of_queue()
    {
        ...
        $reader1 = $this->getMock('Bootphp_Config_Reader');
        $reader2 = $this->getMock('Bootphp_Config_Reader');
    ...
    public function test_detach_removes_reader_and_returns_this()
    {
        ...
        $reader1 = $this->getMock('Bootphp_Config_Reader');
        $reader2 = $this->getMock('Bootphp_Config_Reader', [], [], 'MY_AWESOME_READER');
    ...
    public function test_load_can_get_var_from_dot_path()
    {
        ...
        $reader = $this->getMock('Bootphp_Config_Reader', array('load'));
    ...
    public function test_write_config_passes_changed_config_to_all_writers()
    {
        ...
        $reader1 = $this->getMock('Bootphp_Config_Reader');
        $writer1 = $this->getMock('Bootphp_Config_Writer', array('write'));
        $writer2 = $this->getMock('Bootphp_Config_Writer', array('write'));
    ...
    public function test_config_is_loaded_from_top_to_bottom_of_stack()
    {
        ...
        $reader1 = $this->getMock('Bootphp_Config_Reader', array('load'), [], 'Unittest_Config_Reader_1');
        $reader2 = $this->getMock('Bootphp_Config_Reader', array('load'), [], 'Unittest_Config_Reader_2');

        $reader1
            ->expects($this->once())
            ->method('load')
            ->with($group_name)
            ->will($this->returnValue(array('foo' => 'bar', 'bootphp' => 'awesome', 'life' => array('normal', 'fated'))));

        $reader2
            ->expects($this->once())
            ->method('load')
            ->with($group_name)
            ->will($this->returnValue(array('bootphp' => 'sweet', 'music' => 'tasteful', 'life' => array('extraordinary', 'destined'))));
        ...
        $this->assertSame(
            ...
            'foo' => 'bar',
            ), $config->load($group_name)->asArray()
        );
    }
     ...
     * @test
     * @covers Bootphp_Config::copy
     */
    public function test_copy_copies_merged_config_to_all_writers()
    {
        $config = new Bootphp_Config;

        $reader1 = $this->getMock('Bootphp_Config_Reader', array('load'));
        $reader2 = $this->getMock('Bootphp_Config_Reader', array('load'));

        $reader1
            ->expects($this->once())
            ->method('load')
            ->with('something')
            ->will($this->returnValue(array('pie' => 'good', 'bootphp' => 'awesome')));

        $reader2
            ->expects($this->once())
            ->method('load')
            ->with('something')
            ->will($this->returnValue(array('bootphp' => 'good')));
        ...
        $key = $this->logicalOr('pie', 'bootphp');
