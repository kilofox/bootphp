...
If the above configuration file was called `myconf.php`, you could access it using:
    ...
    $config = Bootphp::$config->load('myconf');
...
We could now call `Bootphp::$config->load('site.title')` to get the site name, and `Bootphp::$config->load('site.analytics')` to get the analytics code.
...
You could then do the following:
	...
	// In your controller
	$view->versions = Bootphp::$config->load('versions');
